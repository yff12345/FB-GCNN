...
def coarsen(A, levels, self_connections=False):
    ...
    '''
    levels = 4
    graphs[0].toarray().shape   (62, 62)
    graphs[1].toarray().shape   (35, 35)
    graphs[2].toarray().shape   (18, 18)
    graphs[3].toarray().shape   (9, 9)
    graphs[4].toarray().shape   (5, 5)
    '''
...
def metis(W, levels, rid=None):
    ...
    N, N = W.shape  # N 62
    if rid is None:
        rid = np.random.permutation(range(N))   # 随机排序(1-62)
    parents = []
    degree = W.sum(axis=0) - W.diagonal()   # D: sum of the cols or rows
    graphs = [W]
...
def perm_data(x, indices):
    ...
    N, M = x.shape      # N: 5  M: 62
    Mnew = len(indices) # 96/80
    # assert Mnew >= M
    xnew = np.empty((N, Mnew))  # xnew.shape = (5, 96)
